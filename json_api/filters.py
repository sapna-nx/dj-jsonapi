...
class RelatedOrderingFilter(OrderingFilter):
    ...
    def translate_field(self, field):
        """
        Translate the ordering parameter to be compatible with django's
        filtering/ordering syntax. This allows you to 
        """
    ...
    def is_valid_field(self, queryset, field, view):
        ...
        """
        # ensure the view is properly defined with serializer and rels.
        ...
        if related:
            ...
            relate_view = view.get_viewset(rel)()
            return self.is_valid_field(queryset, related, related_view)

        # handle field ordering
        else:
            # valid fields are defined in the serializer and relationships
            if not field in [
            ...
            ] + [rel['accessor_name'] for rel in view.relationships]:
                return False

            valid_fields = info.fields_and_pk.keys() + info.relations.keys()
